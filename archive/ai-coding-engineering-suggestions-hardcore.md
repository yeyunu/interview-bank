...
- **Codex 原生可以完成的部分**：阅读代码、进入 Plan 模式、拆任务、修改文件、运行测试、检查 Diff 和进行代码审查。
- **需要项目工程接入的部分**：强制禁止某些写入、自动限制 Diff、拦截危险命令、保存结构化轨迹、在测试失败时阻止任务结束。这些通常需要 `AGENTS.md`、脚本、沙箱、Hook、CI 或自定义 Harness。

因此，文中出现某种配置或 Hook 时，不代表安装 Codex 后会自动生效。它首先描述一种工程机制；只有把机制接入执行链路，它才会从“建议”变成“硬门禁”。

## 1. 用"可执行任务契约"替代自然语言需求

### 通俗理解

任务契约就是给 Codex 一张“施工单”，提前写清楚要修什么、不能碰什么、怎样才算完成。没有施工单时，Codex 只能结合常见做法和现有代码自行推断；推断可能合理，却不一定符合真实业务要求。

例如只说“修复分页”，Codex 不知道页码从 0 还是 1 开始，也不知道能否修改公共 API。任务契约则把答案固定下来：`page=1` 是第一页、函数签名不能变、现有测试不能改、必须新增回归测试。这样 Codex 的自由度被保留在“怎么实现”，而不是扩展到“需求到底是什么”。

它类似维修车辆时的工单：修刹车不等于可以顺便更换发动机；试车通过也不能靠修改仪表盘让故障灯消失。

### Codex 如何落地

Codex 不要求安装 Spec Kit，也不要求一定使用 YAML。小任务直接在提示词里写四项即可：

```text
目标：修复 page=1 返回第二页的问题。
上下文：实现位于 src/pagination.py。
限制：不修改函数签名和已有测试。
完成条件：新增回归测试，pytest 和 ruff 通过。
```

大型任务可以把相同信息放进 GitCode Issue、`PLANS.md`、设计文档或任务 YAML。Codex可以读取并遵守；如果要做到“修改受保护文件就必然失败”，还需 CI 或脚本读取这些规则。也就是说，Codex 能执行契约，但契约的强制性来自项目门禁。
...
Codex 可以在 Plan 模式中完成纵向切分，不需要 Spec Kit。关键是明确要求它按“用户可观察结果”而不是按目录和技术层组织计划：

```text
/plan

请按端到端用户能力拆分任务。每个切片必须能够独立演示、独立测试、单独提交和回滚。
一次只实施一个切片；完成测试和 Diff 检查后，才进入下一个切片。
如果必须先建设共享基础设施，请证明它是第一个切片的阻塞依赖，并保持最小范围。
```

Codex 可以生成这样的计划并逐项执行；但“超过 8 个文件必须停止”之类的 Diff 预算，如果只写在提示词中仍是软约束。要强制执行，需要把检查脚本接入 CI、提交前检查或任务结束门禁。
...
这条建议是在区分“先看病”和“直接开刀”。探索阶段负责弄清代码现状，计划阶段负责决定改法，实施阶段才真正写文件。如果三个阶段没有边界，代理可能刚看到一个函数就开始修改，后来发现真正原因在别处，只能继续打补丁掩盖早期错误。

例如认证故障实际由反向代理 Header 丢失引起，但代理先改了 Token 校验。后续即使发现代理配置问题，错误的 Token 修改已经进入 Diff，又会影响测试和上下文。先只读探索调用链，可以避免这种路径依赖。

### Codex 如何落地

Codex 原生 Plan 模式可以先读取代码、澄清问题和输出计划；也可以通过只读权限阻止计划阶段写入。最简单的指令是：

```text
/plan
先分析调用链、影响范围和验收方案。列出必须由我决定的问题；确认计划前不要修改代码。
```

计划确认后再切换到实施。对于高风险项目，仅靠“不要修改”还不够，应让计划阶段运行在只读沙箱中。文中的 `phase` 状态文件和计划哈希属于更严格的自定义 Harness 设计，不是 Codex 每次任务都会自动生成的文件。
...
模型每次工作前都要先阅读系统规则、工具定义和项目说明。如果这些稳定内容每轮都发生无意义变化，缓存就很难复用，相当于员工每天到岗都要重新参加完整入职培训。

“模型黏性”则是：一个任务开始后，尽量由同一模型继续完成。不同模型的工具习惯、推理方式和对历史内容的理解可能不同；在同一任务中频繁切换，像工程做到一半连续换负责人，交接本身会产生成本和风险。

### Codex 如何落地

普通 Codex 用户通常不需要自己编写路由器。保持同一个任务在线程中连续完成，并让 `AGENTS.md`、工具定义和稳定项目说明保持简洁、顺序稳定，已经能获得主要收益。

只有自建多模型网关或 Harness 时，才需要实现 `route_once()` 和会话模型绑定。需要强调：模型黏性是降低漂移和提高缓存局部性的策略，不是绝对规则。模型不可用、能力不足或任务阶段明显变化时可以切换，但切换后应重新注入关键约束并重新运行验证。
...
上下文不是资料越多越好，而是要让与当前决策最相关的信息占据注意力。修复订单取消时，真正需要的可能只有取消入口、服务方法、仓储接口、状态类型和相关测试；把整个前端资源、生成代码、依赖包和数万行日志同时塞进去，只会把关键线索淹没。

它像医生看膝盖疼：需要病历、膝关节影像和相关化验，不需要把患者一生所有消费记录都打印出来。资料过多不仅更慢，还可能因为同名旧实现、废弃文档和测试夹具产生误导。

### Codex 如何落地

Codex 可以先使用代码搜索或 CodeGraph 找到目标符号、调用者、被调用者和测试，再读取这组最小文件。可以这样要求：

```text
先围绕 OrderService.cancel 建立上下文：定义、直接调用者、直接被调用者、相关类型和测试。
不要读取 vendor、dist、generated 和无关模块。先列出准备读取的文件及理由，再分析问题。
```

你当前项目已经配置 CodeGraph，结构问题应优先用调用图和影响分析定位，而不是递归读取全仓库。自动 token 预算、日志分段压缩和上下文清单，则需要 Codex运行器或自定义 Harness 支持。
...
工具 Schema 相当于标准申请表，结果信封相当于标准回执。没有标准表格时，代理可能把超时时间写成文字、漏掉文件路径，或者把多个目标塞进一个无法解析的字符串。没有标准回执时，工具只返回一大段文本，代理很难稳定判断它究竟成功、失败还是只完成了一部分。

例如测试工具返回“2 failed, 41 passed”这句话，人能理解，但程序若只按文本猜测，可能忽略退出码。统一结果信封直接给出 `ok=false`、`exit_code=1`、日志路径和是否截断，后续决策就不依赖语言猜测。

### Codex 如何落地

Codex 自带工具已经具有结构化参数和结果；普通项目不必重新包装所有内置工具。这个建议主要针对自建 MCP、脚本工具或代理网关。定义工具时应限制必填字段、路径、枚举、数组长度和超时，并让错误返回稳定字段。

Codex 可以读取失败结果并改变方案，但“相同调用失败 5 次后强制熔断”通常需要工具层或 Harness 计数。仅在 `AGENTS.md` 写“不要重复无效命令”能减少问题，却不能提供确定性保证。
...
PreToolUse 的意思是“工具执行前先过门卫”。代理准备写文件、运行命令或访问网络时，门卫先检查操作是否在允许范围内，再决定放行、询问用户还是直接拒绝。

提示词里的“不要删除重要文件”更像墙上的安全标语；执行前检查才像真正的门禁卡。即使模型理解错了、仓库文档含有恶意指令，或者外部网页诱导它读取密钥，门禁仍可独立阻止操作。

例如代理准备写入 `../../.ssh/id_rsa`。仅检查文件名可能看不出问题，但把路径解析成绝对路径后，会发现它已经逃出工作区，应在执行前拒绝。

### Codex 如何落地

`PreToolUse` 是 Claude Code 的事件名称，不应理解为 Codex 必然提供完全同名的接口。Codex 中可以用沙箱、审批策略、项目 Hook、命令包装器和 CI 实现等效边界：

```text
沙箱：限制只能写工作区。
审批：联网、安装依赖或工作区外写入前询问。
命令包装器：只开放允许列表中的发布或迁移命令。
CI：拒绝受保护文件和验证器发生变化。
```

Codex 能遵守 `AGENTS.md` 中的安全规则，但真正的安全边界必须独立于模型。规则即使没有进入上下文，也应继续生效。
...
PostToolUse 就像生产线每完成一道工序立即做小检查。修改 Python 文件后马上跑该文件的 lint 和类型检查；修改数据库迁移后立即检查升级和回滚；修改 Markdown 后立即检查格式与链接。

这样既不必每改一行就运行半小时全量测试，也不会等任务结束才发现第一步已经写错。它解决的是“反馈太晚”的问题。

例如 Codex 把函数返回类型从 `Order` 改成 `Optional[Order]`。如果马上运行 mypy，它可以在下一轮修复调用方；如果最后才检查，错误可能已经扩散到接口、页面和测试。

### Codex 如何落地

`PostToolUse` 同样是 Claude Code 的事件名称。Codex 可以主动在修改后运行局部检查，也可以由项目 Hook 或文件监听器自动触发。最轻量的做法是在 `AGENTS.md` 写清映射：

```text
修改 src/api/**/*.py 后运行 ruff、mypy 和 tests/api。
修改 migrations/** 后运行迁移检查。
修改 docs/** 后运行 markdownlint 和链接检查。
```

Codex 能按规则执行；若要求“每次写入后一定触发”，则需要机械 Hook。增量检查用于快速反馈，任务结束前仍需执行完整验证，不能用局部通过代替全量门禁。
...
Stop Hook 相当于工程出口的验收闸机。代理说“做完了”只是申请离场，闸机还要检查测试、构建、任务清单和受保护文件；证据不完整就退回继续处理。

它类似考试不能由考生自己宣布及格。考生可以说“我答完了”，但最终是否通过由评分规则决定。AI Coding 中的评分规则就是测试结果、Diff 范围和验收条件。

### Codex 如何落地

Codex 原生可以按提示执行“修改—测试—Review”循环，也可以在结束前汇报验证命令和结果。最简单的任务要求是：

```text
不要以文字结论作为完成依据。结束前必须运行指定检查、检查最终 Diff，并列出未通过项；任何必需检查失败时不得声称完成。
```

但这仍属于模型行为约束。`Stop Hook` 是 Claude Code 的具体机制；在 Codex 中要实现同等硬门禁，可以用外部运行器、项目 Hook、CI 或封装脚本：只有验证命令退出码为 0 才接受任务。所有自动继续机制都必须有最大轮次和时间预算，否则一个无法修复的环境错误会造成无限循环。
...
这条建议的意思是：先用便宜、稳定、答案明确的工具检查，再请 AI 和人判断复杂问题。语法错误不需要高级模型讨论，类型错误也不需要 Reviewer 猜测；只有设计合理性、业务遗漏和安全语义等问题，才值得使用语义审查。

它像机场安检：先过证件、行李扫描和金属探测等标准检查，发现明确违禁品直接处理；只有机器无法判断的异常，才交给人工复核。如果一开始把所有行李都交给专家凭经验观察，速度慢且结果不稳定。

### Codex 如何落地

Codex 可以自己运行从格式、类型到测试的检查，再使用 `/review` 或独立会话审查最终 Diff。推荐顺序是：

```text
format/lint → typecheck → unit/integration tests → build → security/performance checks → AI Review → 人审
```

语义 Reviewer 最好不继承实现阶段的整段对话，只读取需求、最终 Diff 和验证结果，避免被实现代理的解释锚定。若第一层已经失败，应先修复，不必立即调用高成本 Reviewer。统一 `make verify`、CI 流程和结构化评分结果需要项目配置，Codex 可以调用它们，但不会替项目自动定义正确的质量门槛。
...
代码覆盖率回答的是“测试有没有经过这行代码”，变异测试回答的是“这行代码如果被改坏，测试能不能发现”。前者像摄像头拍到保安巡逻过仓库，后者则是在受控条件下故意打开一扇门，检查报警器是否真的响。

例如分页实现是 `(page - 1) * size`。变异工具故意改成 `page * size`；如果所有测试仍然通过，说明测试虽然执行了分页函数，却没有检查第一页的真实结果。

### Codex 如何落地

Codex 能安装或调用变异测试工具、分析幸存变异并补充断言。普通业务不必对全仓库运行，优先用于权限、计费、状态迁移、边界计算等关键逻辑：

```text
请针对订单取消逻辑运行变异测试。对每个幸存变异解释：它是等价变异、不可达代码，还是测试缺口；只为真实测试缺口补充断言。
```

变异测试只是额外传感器，不能替代正常单元测试。全仓运行可能很慢，也会出现等价变异，因此阈值需要根据模块风险设定，而不是盲目要求 100%。
...
# pyproject.toml
[tool.mutmut]
source_paths = ["src/order/"]
pytest_add_cli_args_test_selection = ["tests/order/"]
```
...
> **注意**：Mutmut 不同版本的 CLI 和配置字段可能变化。本文采用当前 README 展示的 `source_paths` 配置与 `mutmut run`/`mutmut browse` 工作流；实际接入前仍应以项目锁定版本的文档为准。

## 12. 把架构原则写成"适应度函数"

### 通俗理解

架构适应度函数就是把“代码应该怎么分层”写成可以自动执行的测试。文档里写“领域层不能依赖数据库实现”，人和 Codex 都可能忘记；如果 CI 能检查 import 关系，违规依赖一出现就会失败。

它像建筑规范：不能只在培训里说“承重墙不能随便拆”，还要让施工图审查和现场验收真正阻止违规。架构规则也是如此，只有可执行才不会随着项目变大逐渐失效。

### Codex 如何落地

Codex 可以先使用 CodeGraph 分析当前调用和依赖关系，再帮助编写 Import Linter 契约或结构测试。例如：

```text
分析 app.domain、app.api 和 app.infrastructure 的依赖方向，找出现有违规边；先给出规则和迁移方案，再新增自动架构测试。不要通过把模块加入忽略列表掩盖现有问题。
```

Codex 能生成和运行这些检查，但必须由团队决定哪些边界是业务架构的一部分。规则过严会迫使开发者绕路，规则过松则没有价值。例外应显式记录原因和期限，不能遇到失败就删除检查。
...
普通测试检查“生成的代码对不对”，轨迹回归还检查“代理是怎么做到的”。两个代理都可能让测试通过，但一个只读取相关文件、修改两处并运行验证；另一个读取全仓库、调用工具上百次、修改测试后获得假通过。只看最终结果，会把后者误判为成功。

它像考察司机：不能只看是否到达终点，还要看有没有闯红灯、逆行、超速和绕行几十公里。Harness 的工具调用、审批、token、修改范围就是驾驶轨迹。

### Codex 如何落地

普通 Codex 用户可以先维护一组小型黄金任务：明确输入仓库、目标、禁止修改文件、验收命令和预算；模型、规则或工具升级后重新运行并比较结果。Codex 可以执行这些场景，但自动比较轨迹需要保存结构化事件。

如果没有自建 Harness，也可以从较轻量的指标开始：最终 Diff、修改文件数、测试是否运行、是否修改受保护文件、重复失败命令次数。不要把 token 更少简单等同于质量更高；预算用于发现异常，正确性仍由验收条件判断。
...
长任务状态机就是给任务建立“存档点”。聊天记录告诉你发生过什么，状态文件则明确告诉系统：现在在哪个阶段、哪些步骤已经完成、哪些验证仍然失败、从哪个提交可以恢复。

它类似游戏存档或快递状态：包裹必须按“已揽收→运输中→派送中→已签收”迁移，不能因为快递员说送完了就从运输中直接跳到已签收。AI 任务也不应从“正在实现”直接跳到“完成”，中间至少要经过验证。

### Codex 如何落地

单会话任务可以使用 Codex 的计划和任务状态；跨会话、跨天或涉及数据库迁移的工作，最好把关键状态写入 `PLANS.md` 或专用 JSON：

```text
每完成一个阶段，更新 PLANS.md：完成项、当前项、验证结果、已改文件、剩余风险和恢复命令。新会话开始时先核对 Git HEAD 与计划记录，再继续执行。
```

Codex 能维护这些文件，但原子写入、revision 冲突检测和外部副作用去重需要脚本或 Harness。例如迁移已经执行过一次，恢复后不能因为聊天记录丢失再次执行；应通过迁移表或幂等键判断真实状态。
...
多个代理共用一个目录，就像多名维修工同时在一张工作台上拆同一台机器：一个人刚换好的零件可能被另一个人覆盖，暂存区也分不清是谁的修改。Worktree 为每个任务提供独立工作目录和分支，相当于给每名维修工单独工作台。

文件所有权则进一步说明谁可以碰哪些区域。例如认证代理负责 `src/auth/**`，订单代理负责 `src/order/**`；如果两者都要修改 `pyproject.toml`，该文件属于共享资源，需要集中协调。

### Codex 如何落地

Codex 可以创建和使用 Git worktree，也可以在桌面应用中按独立任务管理不同工作树。适合并行的任务必须边界清晰，例如“补充订单查询测试”和“修复认证日志脱敏”；两个任务都修改同一公共接口时，串行通常更安全。

可以要求每个任务交付：独立 commit、Diff 摘要、验证命令和结果。Worktree 只隔离文件，并不会自动解决接口语义冲突；任务拆分、共享文件审批和最后的集成测试仍然由主流程负责。
...
AI Memory 不是“什么都记住”越好。错误、过期或只在一次故障中临时成立的信息，如果永久保留，会在未来不断误导代理。可靠记忆应该像数据库记录：知道是谁写的、依据是什么、适用于哪里、什么时候过期，以及是否被新版本替代。

例如记忆里写“项目统一使用 MySQL”，三个月后系统已经迁移到 PostgreSQL。如果没有版本和证据，未来 Codex 仍可能生成 MySQL 语法。给记忆附上代码位置、ADR 和有效期后，恢复时就能重新验证。

### Codex 如何落地

Codex 的 `AGENTS.md` 适合保存稳定规则，`PLANS.md` 适合保存任务状态，ADR 和项目文档适合保存架构决策；不要把所有内容都塞进一个无限增长的 Memory 文件。

Codex 可以在任务结束时提出“候选记忆”，但进入长期规则前应由代码或人工确认。建议把事实与偏好分开：测试命令、目录职责可从仓库验证；“以后所有接口都采用 GraphQL”则是尚未批准的决策，不能自动晋升为长期规则。自动 TTL、冲突合并和并发 revision 需要专门的记忆存储层。
...
聊天文本只告诉你代理说了什么，轨迹还要记录它调用了什么工具、输入是否合规、工具返回什么、何时压缩上下文、何时请求批准、测试在哪一轮失败。它相当于飞机的黑匣子：事故发生后不能只看飞行员最后一句话，需要还原完整事件顺序。

例如任务花费突然增加十倍，只有总 token 无法判断原因。轨迹可以区分：是读取了超大日志、缓存持续失效、相同测试重复运行，还是上下文压缩后反复重新探索。

### Codex 如何落地

Codex 已经能显示工具调用和使用量，但团队级比较通常还需要导出或采集结构化日志。最小方案可以保存：任务 ID、模型、提交、工具名称、退出码、耗时、验证结果和 Diff 摘要；成熟方案再增加缓存、压缩、审批和成本事件。

“可回放”不一定意味着重新执行所有危险操作。更安全的方式是保存输入摘要、工具结果和评分工件，离线重新运行评分器。轨迹中可能包含路径、源码和命令，必须脱敏 API Key、令牌和个人数据，并限制日志访问权限。
...
当 Codex 反复犯同一种错误，最简单的反应是在 `AGENTS.md` 或系统提示末尾继续加“请注意”。但规则越来越长后，模型更难找到重点，而且没有证据证明新句子真正解决了问题。

更好的做法是先分类根因：需求没写清就补任务模板；危险命令需要拦截就加权限门禁；测试无效就补测试或变异检查；工具错误难解析就改结果 Schema。提示词只负责表达意图，确定性问题尽量交给程序。

它类似生产事故复盘：不是在墙上再贴一句“操作时请小心”，而是根据事故记录修改设备联锁、检查表或自动报警，并验证同类事故是否真的减少。

### Codex 如何落地

Codex 可以分析失败会话并提出改进，但每项改动应对应一个可复现用例：

```text
请分析这三次失败轨迹，区分需求、上下文、工具、权限和验证问题。每个建议必须说明证据、准备修改的 Harness 组件、预期改善的场景和可能回归；修改后用相同任务重新验证。
```

如果某条新规则没有提高成功率，或引入大量误拦截，应回退。最终目标不是拥有最长的 `AGENTS.md`，而是让高频检视意见逐步变成测试、lint、Schema、Hook 或 CI 门禁。
